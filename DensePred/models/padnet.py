...
class InitialTaskPredictionModule(nn.Module):
    ...
    def __init__(self, p, tasks, input_channels, intermediate_channels=256):
        ...
        for task in self.tasks:
            ...
            conv_out_ = nn.Conv2d(intermediate_channels, p['tasks_outputs'][task], 1)
...
class PADNet(nn.Module):
    def __init__(self, p, backbone, backbone_channels):
        ...
        self.tasks = p['tasks']
        self.auxilary_tasks = p['auxilary_tasks']
        self.channels = backbone_channels
        self.outfns = get_outfns(self.tasks)
        ...
        for task in self.tasks:
            ...
            conv_out_ = nn.Conv2d(256, p['tasks_outputs'][task], 1)
            heads[task] = nn.Sequential(bottleneck1, bottleneck2, conv_out_)

        self.heads = nn.ModuleDict(heads)
    

    def forward(self, x, feat=False, feat_last=False):
        img_size = x.size()[-2:]
        out = {}
        ...
        x = self.initial_task_prediction_heads(feats)
        for task in self.auxilary_tasks:
            out['initial_%s' %(task)] = self.outfns[task](x[task])
        ...
        for task in self.tasks:
            out[task] = self.outfns[task](F.interpolate(self.heads[task](x[task]), img_size, mode='bilinear'))
        for task in self.auxilary_tasks:
            out['initial_%s' %(task)] = self.outfns[task](F.interpolate(out['initial_%s' %(task)], img_size, mode='bilinear'))

        if feat_last:
            return out, list(x.values())
        elif feat:
            # return out, list(x.values())
            return out, feats
        else:
