...
class MTAN(nn.Module):
    ...
    """
    def __init__(self, tasks, backbone, heads: nn.ModuleDict, 
    ...
    def forward(self, x, feat=False):
        ...
        # Task-specific heads
        out = {task: self.outfns[task](self.heads[task](a_4[task])) for task in self.tasks}
        out = {task: F.interpolate(out[task], img_size, mode='bilinear') for task in self.tasks} 
        
        if feat:
            return out, list(a_4.values())
        else:
            return out
