...
class InitialTaskPredictionModule(nn.Module):
    ...
    def __init__(self, p, auxilary_tasks, input_channels, task_channels):
        ...
        self.decoders = nn.ModuleDict({task: nn.Conv2d(task_channels, p['tasks_outputs'][task], 1) for task in self.auxilary_tasks})
...
class MTINet(nn.Module):
    ...
    def __init__(self, p, backbone, backbone_channels, heads):
        ...
        self.outfns = get_outfns(p['tasks'])    
    ...
    def forward(self, x, feat=False):
        ...
        for scale in out['deep_supervision'].keys():
            for task in self.auxilary_tasks:
                out['deep_supervision'][scale][task] = self.outfns[task](F.interpolate(out['deep_supervision'][scale][task], img_size, mode = 'bilinear'))
        ...
        # Feature aggregation
        for t in self.tasks:
            out[t] = self.outfns[t](F.interpolate(self.heads[t](multi_scale_features[t]), img_size, mode = 'bilinear'))
        if feat:
            return out, list(multi_scale_features.values())
        else:
            return out
