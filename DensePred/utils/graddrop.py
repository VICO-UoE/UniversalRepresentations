...
class GDGrad():
    ...
    def gd_backward(self, objectives):
        '''
        ...
        # pdb.set_trace()
        # grads_ = torch.cat(grads, dim=0).t()
        grads_= torch.stack(grads).t()
        dropped_grad = self._graddrop(grads_)
        # pc_grad = self._project_conflicting(grads, has_grads)
        dropped_grad = self._unflatten_grad(dropped_grad, shapes[0])
        self._set_grad(dropped_grad)
        return

    def _graddrop(self, grads):
